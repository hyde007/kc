function onLoadData(){
	...
	if(url.indexOf('name') != -1){
		...
		$.get( "https://www.reddit.com/r/"+name+"/hot.json", function( data ) {
	  		data.data.children[3].data.title
	  		for(var j=1;j<data.data.children.length;j++){
	  			if(j%2 == 1){
	  			$('#redditnews').append('<div class="row "><div class="col-lg-12 bg-light " ><a href="https://reddit.com'+data.data.children[j].data.permalink+'" target="_blank">'+data.data.children[j].data.title+'</a></div></div>');
		  		}else{
					$('#redditnews').append('<div class="row "><div class="col-lg-12 bg-dark text-white " ><a href="https://reddit.com'+data.data.children[j].data.permalink+'" target="_blank">'+data.data.children[j].data.title+'</a></div></div>'); 		
				}	
	  		}
